...
class main_class(QWidget):
	...
	def render_subtitles(self, hide = False, redraw = False):
		...
		h = self.subtitles.height = self.subtitles.geometry().height()
                ...
                #changed
		x = (config.screen_width/2) - (w/2) + 1920
		# x = (config.screen_width/2) - (w/2) # TODO no of monitors on the left
	...
	def render_popup(self, text, x_cursor_pos, is_line):
		...
		if x_cursor_pos == -1:
			x = (config.screen_width/2) - (w/2) + 1920
                        #TODO x += sum of width of before this screen
		else:
			x = x_cursor_pos - w/5
			if x+w > config.screen_width + 1920: #TODO + sum of width of before this screen
				x = config.screen_width - w + 1920 #TODO + sum of width of before this screen
		...
		# self.popup.setGeometry(x, y, w/3, 0)
		self.popup.show()
